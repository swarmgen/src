...
from new_tools import *  # ADDITIONAL TOOLS!!!
...
# random seed configuration is placed on new_tools.py

# RRT algorithm


def giveBuffer(obstacle):

    t_buffer = 0.3

    bigger_obstacle = [[obstacle[0][0] - t_buffer, obstacle[0][1] - t_buffer],
                       [obstacle[1][0] + t_buffer, obstacle[1][1] - t_buffer],
                       [obstacle[2][0] + t_buffer, obstacle[2][1] + t_buffer],
                       [obstacle[3][0] - t_buffer, obstacle[3][1] + t_buffer]]

    return bigger_obstacle
...
def isCollisionFreeVertex(obstacles, xy):
    ...
    for obstacle in obstacles:
        ...
        hull = path.Path(giveBuffer(obstacle))
        collFree = not hull.contains_points([xy])
        if hull.contains_points([xy]):
            # print 'collision'
            # print("collFree: "+str(collFree)+" obstacle: "+str(obstacle))
            return collFree
...
def rrt_path(obstacles, xy_start, xy_goal, params):
    print(f"RRT is working from Starting point{xy_start} to Goal{xy_goal}.")
    ...
    # Convergence criterion: success when the tree reaches within 0.25 in distance from the goal.
    minDistGoal = params.minDistGoal
    # Extension parameter: this controls how far the RRT extends in each step.
    d = params.extension
    ...
    seednumber = params.rand_seed
    # print("This time randomseed(rrt_path) is ["+str(seednumber)+"]")
    np.random.seed(seednumber)  # if put some number in (), it works as it.
    ...
    while not nearGoal:  # and iters < maxiters:
        ...
        rnd = float(np.random.rand(1))  # float
        ...
        if rnd < params.goal_prob:
            ...
            xy = np.array([float(np.random.rand(1))*2*params.world_bounds_x[1]-params.world_bounds_x[1], float(
                np.random.rand(1))*2*params.world_bounds_x[1]-params.world_bounds_x[1]])  # Should be a 2 x 1 vector
        ...
        if not collFree:
            iters += 1
            continue
        if params.animate_rrt and params.visualize:
            # plt.plot(xy[0], xy[1], 'ro', color='k')
            plt.plot(new_node.p[0], new_node.p[1], 'bo',
                     color='blue', markersize=5)  # VERTICES
            plt.plot([closest_node.p[0], new_node.p[0]], [
                     closest_node.p[1], new_node.p[1]], color='blue')  # EDGES
            plt.draw()
            plt.pause(0.01)
        ...
        if norm(np.array(xy_goal) - np.array(new_node.p)) < minDistGoal:
            # print(str(norm(np.array(xy_goal) - np.array(new_node.p)))+"] xy_goal: "+str(xy_goal)+", new_node.p: "+str(new_node.p)+", minDistGoal:"+str(minDistGoal))
            # Add last, goal node
            goal_node = Node()
            goal_node.p = xy_goal
            goal_node.i = len(rrt)
    ...
    if params.visualize:
        plt.plot(P[:, 0], P[:, 1], color='green',
                 linewidth=5, label='path from RRT')

    return P


def ShortenPath(P, obstacles, params, smoothiters=10):
    # INPUTS
    ...
    seednumber = params.rand_seed
    # print("This time randomseed(ShortenPath) is ["+str(seednumber)+"]")
    np.random.seed(seednumber)  # if put some number in (), it works as it.

    m = P.shape[0]
    l = np.zeros(m)
    for k in range(1, m):
        # find all of the straight-line distances
