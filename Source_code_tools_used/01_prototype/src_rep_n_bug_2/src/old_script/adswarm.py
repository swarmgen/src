...
"""
USAGE:
    $ python2 src/swarm_slicing/planner.py adswarm -k vanilla -rs 1 -re 2 
...
def fuzz_one(args, obstacle_type, feedback_type,  check_crash=False, manual_mode=False, param_setting='None', randometest_order=1):
    ...
    '''
    Special move for designated obstacle:
    if this part is activated,
    after rtt works, designated osbtacle is moved to designated place
    before simulation main loop begins.
    
    This is mainly for simulation of waypoint.
    '''
    if SPECIAL_MOVE_OBS_IN_SIM:
        OBSTACLES[5] = move_special_obs_in_sim(OBSTACLES[5])
    ...
    for x in tqdm(range(MAX_TICK)):
        ...
        if simul_tick == p_tick and manual_mode is False:  # TODO

            # print_hdr(simul_tick, p_tick)  # TODO remove this
...
simul_tick [%d] with [%f] in zone_[%d] speedX [%f] obsSize [%f]" % \
        ...
        if len(OBSTACLES) > 2 and feedback_type != "vanilla":
            ...
            if strategy == 'a':
                ...
                # TODO: current version is fixed like this shit
                goal_for_atk_1[0] = 0.0
                # TODO: current version is fixed like this shit
                goal_for_atk_1[1] = 2.0
...
simul_tick [%d] in zone_[%d] at %f %f %f %f" % \
        ...
        new_condition_3 = norm(
            centroid - robots[1].sp) <= 0.4  # new_condition  # 2 * 0.8
...
simul_tick [%d] with [%f] in zone_[%d] speedX [%f] obsSize [%f]" % \
    ...
    if params.mode_record_trajectory_replay is True:
        ...
        npy_save(folderName, params,
                 com_robot_1_sp_global, com_robot_1_sp_global,
                 com_robot_3_sp_global, com_robot_4_sp_global,
                 com_robot_1_sp, com_robot_1_sp,
                 com_robot_3_sp, com_robot_4_sp,
                 com_obstacles)
...
def AdSwarm(feedback_type, rand_start, rand_end):
                ...
                for z in variation_p_purpose:
                    for obstacle_type in obstacle_set:
                        ...
                        logging.debug(
                            "##################################################")
                        logging.debug(
                            str(seed_idx)+"th randometesting.................. ")
                        logging.debug(
                        ...
                        fuzz_one(args, obstacle_type, feedback_type,
...
if __name__ == '__main__':
    ...
    if args.action == "adswarm":
        ...
        if kind == "normal":
            ...
            AdSwarm("dcc", rand_start, rand_end)
            ...
            AdSwarm("vanilla", rand_start, rand_end)
