...
class Robot:
    ...
    def new_local_planner(self, obs, params):
        ...
        # writeFile("temp1.txt", obstacles_grid)
...
def contribution_leader(robots, simulation_tick, params, OBSTACLES, target_obs):
    ...
    temp_sp_leader.append(
        m_original_new_local_planner(
            self_sp, self_sp_global_prime, wall_copied, params))
    ...
    temp_leader_wall = np.linalg.norm(
        after_normal_cal_sp - temp_sp_leader[4])  # new_added

    temp_dcc_sum = temp_leader_global_sp + temp_leader_obs_btt + \
        temp_leader_obs_dia + temp_leader_obs_ltr + temp_leader_wall
    temp_dcc_des = temp_leader_global_sp / temp_dcc_sum
    temp_dcc_1st = None
    temp_dcc_2nd = None
    temp_dcc_3rd = None
    temp_dcc_obs_btt = temp_leader_obs_btt / temp_dcc_sum
    temp_dcc_obs_dia = temp_leader_obs_dia / temp_dcc_sum
    temp_dcc_obs_ltr = temp_leader_obs_ltr / temp_dcc_sum
    temp_dcc_wall = temp_leader_wall / temp_dcc_sum
    # print(f"[DBG] temp_dcc_sum = {temp_dcc_sum}")
    robots[0].influence_max.append(
        [temp_leader_global_sp, temp_leader_obs_btt, temp_leader_obs_dia, temp_leader_obs_ltr, temp_leader_wall])

    robots[0].dcc.append([temp_dcc_des, temp_dcc_obs_btt,
                         temp_dcc_obs_dia, temp_dcc_obs_ltr, temp_dcc_wall])
...
def write_influence(robots, arg_target_index, params):
    ...
    for tick_index in range(len(robots[target_index].influence_max)):
        ...
        if robots[target_index].influence_max[tick_index][0] != 0.00 or robots[target_index].influence_max[tick_index + 1][5] != 0.00:
            ...
            # TODO: later fixed version should have this
            if target_index == 0:
                writeFile(CONTR_PN, str(params.rand_seed) +
                          " " + str(target_index) +
                          " " + str(tick_index+1) +
...
def make_perturbation(current_spawning_point, mode):
    ...
    elif mode == 'big_pure_random':  # 'small'
        perturbed_coord[0] = (
            REGEN_BOUNDARY * rnd_length) * math.cos(rnd_theta)
        perturbed_coord[1] = (
            REGEN_BOUNDARY * rnd_length) * math.sin(rnd_theta)

    elif mode == 'small':  # 'small'
        perturbed_coord[0] = current_spawning_point[0] + \
...
def random_attack_target_gen(params):
    temp_random = float(np.random.rand(1))

    if temp_random >= 0 and temp_random < 0.25:
        params.attack_target = 0
    elif temp_random >= 0.25 and temp_random < 0.5:
        params.attack_target = 1
    elif temp_random >= 0.5 and temp_random < 0.75:
        params.attack_target = 2
    else:
        params.attack_target = 3

    logging.debug("ATTACK_TARGET is : [%f][%i]" %
                  (temp_random, params.attack_target))


def effectWind(position, wind_direct=WIND_DIRECTION, wind_power=WIND_POWER, wind_area_x=WIND_AREA_X, wind_area_y=WIND_AREA_Y):

    if isPosInArea(position, WIND_AREA_X, WIND_AREA_Y):

        theta = 0
        # rnd = float(np.random.rand(1))
        rnd = float(random.random())
        rnd_theta = 1.0/3.0 * math.pi * rnd

        if wind_direct == "east":
            theta = 5.0/6.0 * math.pi + rnd_theta
        elif wind_direct == "west":
            theta = 11.0/6.0 * math.pi + rnd_theta
        elif wind_direct == "south":
            theta = 2.0/6.0 * math.pi + rnd_theta
        elif wind_direct == "north":
            theta = 8.0/6.0 * math.pi + rnd_theta
        else:
            print("error")

        delta_wind_x = wind_power * math.cos(theta)
        delta_wind_y = wind_power * math.sin(theta)

        # print("[dbg] rnd: "+str(rnd) +" theta: "+str(theta) +" delta_wind_x: "+str(delta_wind_x) +" delta_wind_y: "+str(delta_wind_y))
        return np.array([position[0] + delta_wind_x, position[1] + delta_wind_y])
    else:
        return position


def isPosInArea(position, area_x, area_y):
    """_summary_

    Args:
        position (_type_): point [x, y]
        area_x (_type_): [x1, x2]
        area_y (_type_): [y1, y2]
    ...
    Returns:
        _type_: boolean
    """
    if position[0] >= area_x[0] and position[0] <= area_x[1] and position[1] >= area_y[0] and position[1] <= area_y[1]:
        return True
    else:
        return False
