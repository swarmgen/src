...
def making_new_obs_for_moving(OBSTACLE_SET):
    '''
    Inserting
    - Input: TODO
    - Output: TODO
    ...
    delta = 0.3
    new_obstacle = OBSTACLE_SET[1][:][1] + delta

    return new_obstacle


def making_new_obs_for_increasing(OBSTACLE_SET):
    '''
    Inserting
    - Input: TODO
    - Output: TODO
    ...
    delta = 0.3
    new_obstacle = OBSTACLE_SET[1][1][1] + delta

    return new_obstacle


def making_new_obs_for_decreasing(OBSTACLE_SET):
    '''
    Inserting
    - Input: TODO
    - Output: TODO
...
def making_new_obs_for_rotating(OBSTACLE_SET):
    '''
    Inserting
    - Input: TODO
    - Output: TODO
    ...
    delta = 1/6 * math.pi()
    new_obstacle = rotate(OBSTACLE_SET[1], delta)
...
def moving(OBSTACLE_SET, MAP_BOUNDS_METER_new_x, MAP_BOUNDS_METER_new_y):

    while True:
        new_obstacle = making_new_obs_for_moving(OBSTACLE_SET)

        if not isInAreaofAvoid(new_obstacle):
            MOVED_OBSTACLE_SET = OBSTACLE_SET.append(new_obstacle)
            break

    return MOVED_OBSTACLE_SET


def increasing(OBSTACLE_SET, MAP_BOUNDS_METER_new_x, MAP_BOUNDS_METER_new_y):

    while True:
        new_obstacle = making_new_obs_for_increasing(OBSTACLE_SET)
...
def decreasing(OBSTACLE_SET, MAP_BOUNDS_METER_new_x, MAP_BOUNDS_METER_new_y):

    while True:
        new_obstacle = making_new_obs_for_decreasing(OBSTACLE_SET)
...
def rotating(OBSTACLE_SET, MAP_BOUNDS_METER_new_x, MAP_BOUNDS_METER_new_y):

    while True:
        new_obstacle = making_new_obs_for_rotating(OBSTACLE_SET)
...
def isInAreaofAvoid(object):
    ...
    """
    Area to avoid #3: etc <- from pre-defined conf file.
    """
    try:
        xy_ata_x, xy_ata_y = np.genfromtxt("area_to_avoid.txt", delimiter=",")

        for all_four_corners in range(0, 3):
            if new_tools.isPosInArea(object[all_four_corners], xy_ata_x, xy_ata_y):
                print(
                    f"[Report] New generated obstacle is in the area to avoid ({object[all_four_corners]} in [{xy_start_area_x}, {xy_start_area_y}]), so re-gen.")
                return True
    except:
        print("[Report] Set the additional area to avoid.")
