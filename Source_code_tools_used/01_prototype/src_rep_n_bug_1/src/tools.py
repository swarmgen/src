...
def draw_map(obstacles):
    ...
    # Bounds on world
    world_bounds_x = [-MAP_BOUNDS_METER_new_x, MAP_BOUNDS_METER_new_x]
    world_bounds_y = [-MAP_BOUNDS_METER_new_y, MAP_BOUNDS_METER_new_y]
...
def draw_gradient(f, nrows=MAP_SIZE_new_y, ncols=MAP_SIZE_new_x):

    skip = 10
    [x_m, y_m] = np.meshgrid(np.linspace(-MAP_BOUNDS_METER_new_x, MAP_BOUNDS_METER_new_x, ncols),
                             np.linspace(-MAP_BOUNDS_METER_new_y, MAP_BOUNDS_METER_new_y, nrows))
    [gy, gx] = np.gradient(-f)

    Q = plt.quiver(x_m[::skip, ::skip], y_m[::skip, ::skip],
...
def waypts2setpts(P, params):
    ...
    """
    V = params.drone_vel * \
        1.3  # [m/s], the setpoint should travel a bit faster than the robot
    freq = params.ViconRate
    dt = 1./freq
    dx = V * dt
...
def formation(num_robots, leader_des, v, l):
    ...
    des7 = leader_des - v*l*sqrt(3) + u*l
    des8 = leader_des - v*l*sqrt(3) - u*l

    des5 = leader_des - v*l*sqrt(3)*3/2 - u*l/2
    des6 = leader_des - v*l*sqrt(3)*3/2 + u*l/2
    ...
    if num_robots == 5:
        return [des2, des3, des4, des5]
    if num_robots == 6:
        return [des2, des3, des4, des5, des6]
    if num_robots == 7:
        return [des2, des3, des4, des5, des6, des7]
    if num_robots == 8:
        return [des2, des3, des4, des5, des6, des7, des8]
    if num_robots == 9:
        return [des2, des3, des4, des5, des6, des7, des8, des9]
...
def poses2polygons(poses, l=0.1):
    ...
    for pose in poses:
        pose = np.array(pose)
        polygon = np.array([pose + [-l/2, -l/2], pose +
                            [l/2, -l/2], pose + [l/2, l/2], pose + [-l/2, l/2]])
