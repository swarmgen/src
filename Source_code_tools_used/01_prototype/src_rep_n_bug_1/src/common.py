...
class Params:
    def __init__(self):
        ...
        # the RRT algorithm
        self.animate_rrt = VISUALIZE_RRT
        ...
        self.world_bounds_x = [-MAP_BOUNDS_METER_new_x,
                               MAP_BOUNDS_METER_new_x]  # [-2.5, 2.5]

        # [m], map size in Y-direction
        self.world_bounds_y = [-MAP_BOUNDS_METER_new_y,
                               MAP_BOUNDS_METER_new_y]  # [-2.5, 2.5]
        ...
        # near the obstacle
        self.influence_radius = 0.45  # 0.15 -> 0.3 -> 4.5
        ...
        # 5
        self.mode_record_trajectory_replay = False
        # 6
...
def visualize2D(tick, obs, params, robots, robot1, centroid, traj_global):
    ...
    plt.plot(
        robots[0].sp_global[0], robots[0].sp_global[1], '*', color='black', markersize=10,
        label='Centroid position')
    # plt.plot(
